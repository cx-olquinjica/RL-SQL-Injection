<?php
 echo "Welcome";
 echo "<br />";
 //These are the defined authentication environment in the
db service
 $host = ’mysqldb’;
 $user = ’root’;
 ...
 if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $data = $_REQUEST[’email’];
    echo "<br />";
    //Simple filter that detects illegal words and removes
        them from query
    $illegal_words = [" or "];
    #choosing one of the words in the list at random to
   filter out
    $chosen_word = $illegal_words[rand(0, count(
        $illegal_words)-1 )];
    echo $chosen_word;
   ...
   if ($result = $conn->query( "SELECT name, company FROM
